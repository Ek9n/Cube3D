...
/*   Updated: 2024/04/02 11:06:14 by yubi42           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void set_corners_mov(t_player *player, int sign)
{
	...
	player->corners[0][0] = player->x + (IMG_SIZE / 2) + (8 * cos1) - (8 * sin1) + sin1 * sign;
	player->corners[0][1] = player->y + (IMG_SIZE / 2) - (8 * sin1) - (8 * cos1) + cos1 * sign;
	player->corners[1][0] = player->x + (IMG_SIZE / 2) + (8 * cos2 ) - (8 * sin2) + sin1 * sign;
	player->corners[1][1] = player->y + (IMG_SIZE / 2) - (8 * sin2 ) - (8 * cos2) + cos1 * sign;
	player->corners[2][0] = player->x + (IMG_SIZE / 2) + (-8 * cos1 ) - (-8 * sin1) + sin1 * sign;
	player->corners[2][1] = player->y + (IMG_SIZE / 2) - (-8 * sin1 ) - (-8 * cos1) + cos1 * sign;
	player->corners[3][0] = player->x + (IMG_SIZE / 2) + (-8 * cos2 ) - (-8 * sin2) + sin1 * sign;
	player->corners[3][1] = player->y + (IMG_SIZE / 2) - (-8 * sin2 ) - (-8 * cos2) + cos1 * sign;
}
...
int check_mid_wall(t_map *map, int x1, int y1, int x2, int y2, int modx, int mody)
{
	if(modx > mody)
	{
...
int mid_coll(t_map *map, int x1, int y1, int x2, int y2)
{
	...
	if(((x1 / IMG_SIZE) - (x2 / IMG_SIZE)) && ((y1 / IMG_SIZE) - (y2 / IMG_SIZE)))
	{
		...
		printf("1: %i\n", mod_x1 + mod_y1);
		printf("2: %i\n", mod_x2 + mod_y2);
		if((mod_x1 + mod_y1 <= mod_x2 + mod_y2) && check_mid_wall(map, x1, y1, x2, y2, mod_x1, mod_y1))
			return (1);
		else if((mod_x2 + mod_y2 < mod_x1 + mod_y1) && check_mid_wall(map, x2, y2, x1, y1, mod_x2, mod_y2))
			return (1);
	}
	return (0);
}

int calc_coll_angle(t_data *data, t_player *player, float *angle, char side, char dir)
{
	float mod;
	int num;

	mod = ((int)(player->angle * 1000) % (int)(0.5 * PI * 1000));
	mod /= 1000;
	if (dir == 'u')
		num = XK_Up;
	else
...
int	check_corner_collision(t_data *data, t_map *map, t_player *player)
{
	...
	angle = player->angle;
 	if (map->grid[player->corners[0][0] / 64][player->corners[0][1] / 64] == 1)
		return calc_coll_angle(data, player, &angle, 'l', 'd');
	if(map->grid[player->corners[1][0] / 64][player->corners[1][1] / 64] == 1)
		return calc_coll_angle(data, player, &angle, 'r', 'd');
	if(map->grid[player->corners[2][0] / 64][player->corners[2][1] / 64] == 1)
		return calc_coll_angle(data, player, &angle, 'l', 'u');
	if(map->grid[player->corners[3][0] / 64][player->corners[3][1] / 64] == 1)
		return calc_coll_angle(data, player, &angle, 'r', 'u');
	
	if(mid_coll(map,player->corners[2][0], player->corners[2][1], player->corners[3][0], player->corners[3][1]) || mid_coll(map,player->corners[0][0], player->corners[0][1], player->corners[1][0], player->corners[1][1]))
	{
...
/* 	else
		set_corners_mov(data, data->player, sign, num); */
