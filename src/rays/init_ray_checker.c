...
/*   Updated: 2024/04/06 18:30:36 by yubi42           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void init_ray_steps(t_check_ray *ray)
{
	...
	if (ray->fabs_sin < EPSILON)
		...
		ray->first_row_step = (IMG_SIZE - (IMG_SIZE - (ray->x % IMG_SIZE))) / ray->fabs_sin;
	else 
		ray->first_row_step = (IMG_SIZE - (ray->x % IMG_SIZE)) / ray->fabs_sin;
	if (ray->fabs_cos < EPSILON)
		ray->first_col_step = 0;
	else if (ray->angle >= 0.5 * PI && ray->angle < 1.5 * PI)
		ray->first_col_step = (IMG_SIZE - (IMG_SIZE - (ray->y % IMG_SIZE))) / ray->fabs_cos;
	else 
